    ...
    return (
        <>
            <Box onClick={onOpen} cursor={'pointer'} mt={'10px'}>
                <Text color={'red'} fontWeight={500} fontSize={'18px'}>
                    <Icon as={ImSun} /> {Math.round(data.temp.day)}<sup>o</sup> C
                </Text>
                <Text color={'red'} fontWeight={500} fontSize={'27px'}>
                    <Icon as={MdOutlineNightsStay} /> {Math.round(data.temp.night)}<sup>o</sup> C
                </Text>
                <Text color={'red'} fontWeight={500} fontSize={'20px'}>
                    {data.weather[0].main}
                </Text>
            </Box>
                            ...
                            <Grid templateColumns={'50% 50%'} >
                                <Box pb={'10px'} pl={'15%'}>
                                    {['Felt Temp.', 'Humidity', 'Wind', 'Pressure', 'Day Temp.', 'Evening Temp.', 'Night Temp.', 'Max Temp.', 'Min Temp.'].map((e, i) => (
                                        <Text key={i} color={'orange'} fontWeight={500} mt={'15px'} fontSize={'18px'} >{e}</Text>
                                    ))}
                                </Box>
                                <Box borderRadius={'30px'} bg={'orange'} pb={'10px'} pl={'15%'}>
